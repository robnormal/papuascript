...
Assign.funcAssignment = function(func_call, op, body) {
	var
		params = func_call.factors.slice(1),
		name = func_call.factors[0],
		funcName, block;
	
	// don't name member functions
	if (void 0 !== func_call.factors[0].value &&
			-1 === func_call.factors[0].value.indexOf('.')
	) {
		funcName = func_call.factors[0];
	}

	// check that params are all Ids
	for (var i = 0, len = params.length; i < len; i++) {
		if (! (params[i] instanceof Identifier)) {
			throw new Error('Bad parameter in function definition: expected Identifier, found ' + params[i]);
		}
	}
	...
	return Assign.create(name, op, new Code(params, block, funcName));
};
...
(function() {
	// guarantee there are no repeated variable names
	// by using this for all FuncCalls
	var partialParamNum = 0;

	var createPartial = function(funcCall) {
		var args = [], newParams = [], newIdent;

		// first factor is the function being called
		for (var i = 1, len = funcCall.factors.length; i < len; i++) {
			if ('@' === funcCall.factors[i]) {
				newIdent = new Identifier('_p' + partialParamNum);
				args.push(newIdent);
				newParams.push(newIdent);

				partialParamNum++;
			} else {
				args.push(funcCall.factors[i]);
			}
		}
		...
		if (! newParams.length) {
			...
			return new Code(newParams, new Block([
				new FuncCall([funcCall.factors[0]].concat(args))
			]));
		}
	};
		...
		partialCall: function() {
			if (this._partial) {
				...
				for (var i = 0, len = this.factors.length; i < len; i++) {
					if (this.factors[i] === '@') {
						this._partial = createPartial(this);

						return this._partial;
					}
				}

				// no placeholders, so not a partial
				return false;
			}
		}
	});
...
Access.spaceDot = function(x, index) {
	if (index instanceof FuncCall) {
		return new FuncCall(
			[new Value(x).add(new Access(index.factors[0]))]
				.concat(index.factors.slice(1))
		);
	} else if (index instanceof Identifier) {
		return new Value(x).add(new Access(index));
	} else {
		H.throwSyntaxError('Bad index', { first_line: index.getLineno() });
	}
};
...
Switch = function Switch(expr, cases_with_default) {
	this.expr = expr;
	this.cases = cases_with_default[0];
	this.deflt = cases_with_default[1];
}
