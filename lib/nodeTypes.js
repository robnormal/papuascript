...
var
	...
	Identifier, Literal, Index, Arr, FuncCall, Parenthetical,
...
Assign.baseNamesFor = function(assignee) {
	if ($.isArray(assignee)) {
		list_bind(assignee, Assign.baseNamesFor);
...
Operation.create = function(op, a, b) {
	...
	if (params.length) {
		return new Code(params, new Block([new Operation(op, aInFunc, bInFunc)]));
	} else {
...
FuncCall.fromChain = function(call_or_factor, chain) {
	var
		base = call_or_factor,
		accessor, ident, factors, is_call;
...
var Cps = function Cps(expr, code) {
	var fnCall;
