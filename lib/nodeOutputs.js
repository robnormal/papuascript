...
extendPrototypes({
	...
	Import: {
		...
		_assignMembers: function(owner) {
			ls = new Lines([]);
			for (var i = 0, len = this.members.xs.length; i < len; i++) {
				ls.append(this.members.xs[i].lines())
          .suffix(' = ' + owner + '.')
          .append(this.members.xs[i].lines())
          .suffix('; ');
			}
		...
		lines: function() {
			...
			if (this.owner) {
				ls.prefix(this.owner + ' = ')
					.suffix(';')
					.append(this._assignMembers(this.owner));
			} else {
				ls.prefix('(function() { var owner = ')
					.suffix(';')
					.append(this._assignMembers('owner'))
					.suffix('})(); ');
			}
