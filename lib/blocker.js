...
$.extend(Blocker.prototype, {
	...
	fixWhile: function() {
		...
		this.expression(true); // could have an AssignList, then the condition
		this.block();
	},
	...
	processExpression: function(noIndent, startDent, stopWhen) {
		...
		while (this.has()) {
			...
			case '#':
				...
				// don't start expression on TERMINATOR
				if ('TERMINATOR' === this.tag()) {
					this.removeToken();
				}
	...
	isFunctionAssign: function() {
		var i = this.pos, factors = 0;

		if ('ASSIGN' !== this.tagAt(i)) {
			return false;
		}
		i--;

		while (i >= 0 && ! this.isLineBreakAt(i)) {
			if ('IDENTIFIER' !== this.tagAt(i)) {
				return false;
			} else {
				factors++;
				i--;
