/*jshint regexp: false, white: false, indent: false, boss: true */
...
var JS_KEYWORDS, PAPUA_KEYWORDS, RESERVED, STRICT_PROSCRIBED, JS_FORBIDDEN,
		IDENTIFIER, NUMBER  , WHITESPACE, COMMENT   , MULTI_DENT, SIMPLESTR ,
		...
		UNARY_ASSIGN, PAPUA_OPS, TO_EOL, ASSIGN, REG_TEXT_IDENT
		;
...
REG_TEXT_IDENT = '([$A-Za-z_\\x7f-\\uffff][$\\w\\x7f-\\uffff]*)';

IDENTIFIER = new RegExp('^' + REG_TEXT_IDENT);
...
Lexer.prototype = {
	setInput: function(s) {
		this.text = s;
		this.fixed_tokens = B.resolveBlocks(this.tokenize(this.text));
		this.lex_index = 0;
  ...
  // Matches strings, :symbols, and %{ words }
  string: function() {
		var match, prev, rgx, string;

    switch (this.chunk.charAt(0)) {
		case "'":
		case '"':
			rgx = this.chunk[0] === "'" ? SINGLESTR : DOUBLESTR;

			match = rgx.exec(this.chunk);
			if (! match) return 0;
		...
		case ':':
			...
			/* only defines a symbol-string if colon is spaced,
			 * or does not follow a word */
			if (prev && (! prev.spaced && prev[1].match(/[\w\d]$/))) {
				return 0;
			} else {
				match = this.chunk.substr(1).match(IDENTIFIER);

				if (! match) {
					return 0;
				} else {
					this.token('STRING', '"' + match[0] + '"', 0, match[0].length + 1);
	...
	comment: function() {
		...
		if (matchEnd === '/*') {
			...
			return matched_chars +
				this.chunk.substr(matched_chars).match(TO_EOL)[0].length;
		}
	},
  ...
  // can close multiple indents, so we need to know how far in we happen to be.
	line: function() {
		...
		return match[0].length + this.spacedot(match[0].length);
	},
  ...
  // Matches and discards whitespace.
	whitespace: function() {
		...
		return len + this.spacedot(len);
	},

	// check for SPACEDOT; call after consuming whitespace
	spacedot: function(pos) {
		// check for SPACEDOT
		if ('.' === this.chunk[pos]) {
			this.token('SPACEDOT', '.', pos, 1);
			return 1;
		} else {
			return 0;
		}
  ...
  // parentheses that indicate a method call from regular parentheses, and so on.
	literal: function() {
		...
		if ((match = H.find_init(OPERATORS, this.chunk))) {
			if (this.chunk.charAt(match.length).match(OPERATOR_CHAR)) {
				this.error('Unknown operator ' + match + this.chunk.charAt(match.length));
			} else {
				value = match;
			}
		} else {
