...
(function() {
	var
		...
		msg_not_exist = 'Variable "{0}" not in scope: Use := to update variables in the outer scope.',
		...
		opErr = function(node, v, msg) {
			var line = (node.assignee.base && node.assignee.base.value.line) ||
				(node.assignee.value && node.assignee.value.line);
	...
	$.extend(Assign.prototype, {
		...
		checkUpdatedRaw: function(scope, outer_scope, inner_scope) {
			var
				...
				in_outer, in_current, in_inner, v, at_top;
			...
			for (var i = 0, len = vs.length; i < len; i++) {
				...
				if (! (in_outer || in_current || defining) && ! at_top) {
