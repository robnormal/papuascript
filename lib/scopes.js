...
(function() {
	...
	$.extend(Assign.prototype, {
		...
		varsReferenced: function(in_loop, lower) {
			...
			return [new_here.concat(res[0]), new_lower.concat(res[1])];
		},
...
$.extend(Code.prototype, {
	varsReferenced: function(in_loop, elsewhere) {
		...
		var args = L.pluck(this.params, 'value');
		var res = this.block.varsReferenced(false, elsewhere);

		// all references are inner-scope wrt the caller
		return [ [], $.difference(res[0].concat(res[1]), args) ];
	}
});
