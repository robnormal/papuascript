...
$.extend(PNode.prototype, {
	varsDefinedRaw: function() {
		if (this.canDefineVars) {
			return H.collect('varsDefinedRaw', [], this.children());
		} else {
			return [];
		}
	},
	...
	checkUpdatedChildren: function(scope, outer_scope, inner_scope) {
		...
		$.each(this.children(), function(n) {
			if (n instanceof PNode) {
				H.invoke('checkUpdated', [scope, outer_scope, inner_scope], n);
			}
		});
	},
...
$.extend(Script.prototype, {
	...
	checkScope: function() {
		this.checkUpdated(
			null,
			$.map(this.exports, function(ex) { return ex.value; }),
			null
		);
	...
	varsReferenced: function(in_loop, reffed_lower) {
		return PNode.varsReferencedScope(this.body, in_loop, reffed_lower);
	}
});

$.extend(Code.prototype, {
	checkUpdated: function(scope, outer_scope, inner_scope) {
		var params = $.map(this.params, function(p) { return p.value; });
		// arguments to the function are outer variables, as is everything
		// currently in scope
		this.block.checkUpdated(
			[],
			/* variables currently in scope and function parameters
			 * are all external variables */
			scope.concat(outer_scope.concat(params)),
			[]
		);
	},

	/**
	 * @arg {boolean} - whether we are in a loop
	 * @arg {string[]} - variables referenced at a lower scope
	 */
	varsReferenced: function(in_loop, lower) {
		...
		var
			res = this.block.varsReferenced(false, lower),
			args = $.pluck(this.params, 'value'),
		...
		checkUpdatedRaw: function(scope, outer_scope, inner_scope) {
			...
			for (var i = 0, len = vs.length; i < len; i++) {
				...
				if (v === 'this') {
					...
					opErr(this, v, 'Undefined variable: ' + v);

				} else if (!(defining || updating || updating_inner)) {
					opErr(this, v, 'Unrecognized assignment operator: ' + this.op);
			...
			H.invoke('checkUpdated', [scope, outer_scope, inner_scope], this.value);
		}
	});
...
$.extend(N.Obj.prototype, {
	varsReferenced: function(in_loop, lower) {
		return PNode.varsReferencedScope(
			$.pluck(this.props, 1), in_loop, lower
		);
	}
});
...
$.extend(N.Import.prototype, {
	varsDefinedRaw: function() {
		return this.members.xs.concat(this.owner ? [this.owner] : []);
	}
});
